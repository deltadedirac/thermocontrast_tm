    ...
    def train_LLMRegresor(self,train_iterator, val_iterator, model, device, criterion, optimizer, epoch_num, path_progres, checkpoint_path):
        ...
        for epoch in tqdm( range(epoch_num) ):
            ...
            for input, label in tqdm(train_iterator, desc ='training epoch by batches: ', leave=False):
                ...
                # this segment is fot geometric loss contribution
                df_for_geo_loss = train_iterator.dataset.data.loc[input]
                df_for_geo_loss['embedding_poss']=list(range(0,df_for_geo_loss.shape[0]))
                ...
                organism_Tm_batches_train = torch.tensor(train_iterator.dataset.data.loc[input].PrimeOGT.to_numpy())\
                                                                                            .to(dtype=torch.float32, device=device) 
                out = model(data, organism_Tm_batches_train.unsqueeze(1))
            ...
            with torch.no_grad(): # evaluate validation loss here 
                ...
                for (inputval, labelval) in val_iterator:#, desc='model in validation set: ', leave=False):
                    ...
                    # this segment is fot geometric loss contribution
                    df_for_geo_valloss = train_iterator.dataset.data.loc[input]
                    df_for_geo_valloss['embedding_poss']=list(range(0,df_for_geo_valloss.shape[0]))
                    ...
                    # Here the output model is a tuple where: component1: (OGT, Tm bias), component2: embeddings from Light attention
                    organism_Tm_batches_val = torch.tensor(val_iterator.dataset.data.loc[inputval].PrimeOGT.to_numpy())\
                                                                                                        .to(dtype=torch.float32, device=device)
                    outval = model(datat, organism_Tm_batches_val.unsqueeze(1))
    ...
    def test_model(self, model, test_loader, loss, device):
        ...
        with torch.no_grad():
            ...
            for test_setidx, labels in tqdm(test_loader, desc ='testing data: ', leave=False):
                ...
                df_for_geo_testloss = test_loader.dataset.data.loc[test_setidx]
                df_for_geo_testloss['embedding_poss']=list(range(0,df_for_geo_testloss.shape[0]))
                organism_Tm_batches_test = torch.tensor(test_loader.dataset.data.loc[test_setidx].PrimeOGT.to_numpy())\
                                                                                                        .to(dtype=torch.float32, device=device)
                ...
                outcome = model(test_set, organism_Tm_batches_test.unsqueeze(1))
